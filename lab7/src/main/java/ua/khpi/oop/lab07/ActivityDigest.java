...
public class ActivityDigest implements Trackable, Exportable {
    ...
    public String TrackingDetails() {
        return String.format("Активність %s    Виконано завдань %d/%d    Дописів на форумі %d", studentId, completedAssignments, totalAssignments, forumPosts);
    }
    ...
    public String exportData(String format) {
        if ("JSON".equalsIgnoreCase(format)) {
            return String.format("\"studentId\": \"%s\", \"completionRate\": %.2f, \"forumPosts\": %d", studentId, CompletionRate(), forumPosts);
        }
